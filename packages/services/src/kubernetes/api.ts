...
export class KubeApi {
    ...
    constructor(loadFromCluster?: boolean) {
        this.config = new k8s.KubeConfig();

        if (loadFromCluster) this.config.loadFromCluster();
        else this.config.loadFromDefault();
        ...
        return this.coreApi
            ...
            .then((secret) => {
                ...
                for (const key in secret.data) {
                    result[key] = base64Decode(secret.data[key]);
                }
                return result;
            });
    }
