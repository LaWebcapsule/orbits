...
import { Action, ActionApp, ActionState, Workflow } from '@wbce/orbits-core';
import { Cli, KubeApi } from '@wbce/services/index.js';

import * as cdk8s from 'cdk8s';
import { mkdir, rm, writeFile } from 'fs/promises';

export class Cdk8sAction extends Workflow implements cdk8s.IResolver {
    cli = new Cli();
    kubeApi = new KubeApi();
    ...
    async define() {
        ...
        await this.do('Prune', {
            dynamicAction: () => {
                ...
                pruneAction.main = this.pruneMain.bind(this, this.bag.rollback);
                pruneAction.setRepeat({ [ActionState.ERROR]: 2 });
                return pruneAction;
            },
        });

        if (this.bag.rollback) {
            // Reject with error if rollback was set
            // Action will be in error
            throw new Error(this.bag.err);
        }
     ...
     *
     * Secret name is `wbce.${this.argument.stackName}`
     *
     * @returns the secret name
     */
    private genSecretName(): string {
        return `wbce.deployment.${this.argument.stackName}`;
    }
    ...
    async actionWatcher(type: 'deploy' | 'rollback'): Promise<ActionState> {
        const objectsPromise =
            type === 'deploy'
                ...
                : this.retrievePreviousChart().then((chart) =>
                      loadAllYaml(chart)
                  );
            ...
            .then((results) => {
                ...
                for (const status of results) {
                    ...
                    if (status == ActionState.ERROR) {
                        this.internalLogError(
                            new Error('Got an error, terminating')
                        );
                        return ActionState.ERROR;
                    }
                }
...
export class Cdk8sApp extends ActionApp {
    declare = [Cdk8sAction];
}
