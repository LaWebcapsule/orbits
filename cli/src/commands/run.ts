...
const run = async (
    ...
    try {
        ...
        logErrorAndExit(
            `Error running ${colors.italic.bold(actionRef)}:\n${error}`,
...
const processRunCmd = async (actionRef: string, actionArgs: any, opts: any) => {
    ...
    return opts.local
        ? runLocal(actionRef, actionArgs, opts, cliInstanceUUID)
        : run(actionRef, actionArgs, opts, cliInstanceUUID);
};
