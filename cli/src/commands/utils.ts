...
import { deepCopy } from '../../../packages/services/src/utils.js';
...
export const setUpRuntime = async (opts?: {
    ...
    if (!opts?.noDatabase) {
        if (!process.env['ORBITS_DB__MONGO__URL'])
            console.warn(
                `WARN - Env variable ${colors.italic.bold('ORBITS_DB__MONGO__URL')} not set, using default value ${colors.italic(DEFAULT_DATABASE)}`
            );
    }
    ...
    const importActionsFilesPromises = actionsFiles.map(async (file) => {
        const actionPath = !file.startsWith('/')
        ...
        try {
            ...
            logErrorAndExit(
                `Cannot access ${colors.italic.bold(actionPath)}`,
        ...
        try {
            ...
            logErrorAndExit(
                `Error importing ${colors.italic.bold(actionPath)}: ${error}`,
