...
export class Workflow extends Action {
        ...
        if (!actionDbDoc) {
            if (action.constructor[COALESCING_WORKFLOW_TAG]) {
                ...
                if (actionDescriptor) {
                    ...
                    actionDbDoc = (action as CoalescingWorkflow).substitute(
                        actionsWithSameIdentity
                    );
                }
            }
        }
    ...
    async do(
        ...
        try {
            ...
            if (this.defineCallMode === 'main') {
                action.dbDoc.cronActivity.pending =
                    this.dbDoc.cronActivity.pending;
                await action.resume();
