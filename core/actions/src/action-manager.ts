import { orbitsAsyncStorage, utils } from '@orbi-ts/services';
...
export class Action {
    ...
    save() {
        return this.runtime.waitForBootstrap.then(() => {
            ...
            if (
                !(
                    ActionCtr === this.constructor ||
                    ActionCtr.name !== this.constructor.name
                )
            ) {
                throw new ActionError(
            ...
            .then((actionState) => {
                if (
                    ...
                    if (
                        ...
                        const timeoutError = new ActionError('Action has been IN_PROGRESS for too long (property delays of the action can be used to configure the timeout)');
    ...
    notifyHealth() {
        ...
        this.notifyHealthPromise = Promise.resolve()
            .then(() => {
